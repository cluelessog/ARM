...
;I will be using 5 values from the pixels value already stored above starting from 0x20000000 and do encryption on them
	...
	SUB r3,r3,#1; hack
	MOV r11, r3; Saving the address for using in Hamming code
...
loop1	LDRB r4, [r2,#0x1]!; register store the current data to be processed
		...
		CMP r6, #5; comparison to read only 5 values
		BLT loop1;
;Hamming encoding Starts Here

	MOV r3, r11;
	LDR r1, =0x200000ea ; Start storing the hamming encoded data
	SUB r1, r1, #1; hack
	MOV r0, #0; counter
loop2	LDRB r4, [r3,#0x1]!; r4 to store current data to be processed 
	...
	STRB r5, [r1, #0x1]!; r5 holds the intermediate results of encoding and store at address stored in r1
	LSR r5, #8;
	STRB r5, [r1,#0x1]!;
...
;Hamming decoding starts here
		...
		LDR r3, =0x200000ea ; Start storing the hamming encoded data
		SUB r3, r3, #1; hack
		MOV r11, #0; counter
		LDR r12,=0x20000104 ; start address to store the decoded data
		SUB r12, r12, #1; hack
decoderead	LDRB r4, [r3,#0x1]!; r4 to store current data to be processed 
...
STOREINMEM STR r8,[r12,#0x1]!
	...
	LDR r3, =0x20000104; Start address for decription
	SUB r3,r3,#1;
	LDR r2, =0x2000011E;
	SUB r2, r2, #1
...
;decrypt function
	...
	MOV r5, #0x000000ff; IV
	MOV r6, #0; counter
loop3 LDRB r4, [r3,#0x1]!; reading decripted data
	MOV r7, r4;
	EOR r4, r4, r5;
	STRB r4, [r2,#0x1]!; storing the final decripted data at address stored in r2
	MOV r5, r7;
	ADD r6, r6, #1;
	CMP r6, #5;
	BLT loop3;
